...
namespace Funda.Services
{
    public class FundaStat : IFundaStat
    {
        ...
        public async Task<Dictionary<string, int>> GetTopBrokers(bool withTuin, Action<int>? progressCallback = null, int top = 10)
        {
            var saleObjects = await _fundaApi.GetSaleObjects(withTuin, progressCallback);
